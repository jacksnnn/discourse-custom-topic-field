...
after_initialize do
  ...
  on(:topic_created) do |topic, opts, user|
    default_value = 'wwww.defaulturl.com'
    Rails.logger.info "Options passed to topic_created: #{opts.inspect}"
    topic.send("#{FIELD_NAME}=".to_sym, opts[FIELD_NAME.to_sym] || default_value)
    topic.save!
    Rails.logger.info "Custom field value after save: #{topic.custom_fields[FIELD_NAME]}"
  end
